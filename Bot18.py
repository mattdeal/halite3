...
class Bot18:
    ...
    def run(self):
        ...
        # calculate clusters
        tile_values_o = [[0 for x in range(MAP_SIZE)] for y in range(MAP_SIZE)]
        cluster_values_o = [[0 for x in range(MAP_SIZE)] for y in range(MAP_SIZE)]
        for x in range(MAP_SIZE):
            for y in range(MAP_SIZE):
                tile_values_o[y][x] += game.game_map[Position(x, y)].halite_amount
        ...
        halite_total_o = np.matrix(tile_values_o).sum()

        inspiration_values = [[0 for x in range(MAP_SIZE)] for y in range(MAP_SIZE)]
        ...
        while True:
            ...
            halite_total_n = np.matrix(tile_values_n).sum()
            halite_pct_remaining = halite_total_n / halite_total_o * 100
            ...
            for player in game.players.values():
                ...
                for ship in player.get_ships():
                    ...
                    # update inspiration radius around each ship
                    for x in range(ship.position.x - 4, ship.position.x + 4):
                        p_diff = 4 - abs(x - ship.position.x)
                        for y in range(ship.position.y - p_diff, ship.position.y + p_diff):
                            p_norm = game_map.normalize(Position(x, y))
                            inspiration_values[p_norm.y][p_norm.x] += 1
            ...
            for ship in me.get_ships():
                ...
                # if we are trying to avoid collisions, move away from an enemy ship if one moved beside us during last turn
                danger_level = danger_tiles_n[ship.position.y][ship.position.x]
                if ENEMY_RADIUS > 0 and danger_level > 1:
                    logging.info("Ship {} at position {} has {} danger.".format(ship.id, ship.position, danger_level))

                    nearby_positions = ship.position.get_surrounding_cardinals()
                    less_dangerous_tiles = []
                    for position in nearby_positions:
                        ...
                        if danger_tiles_n[p_norm.y][p_norm.x] < danger_level:
                            less_dangerous_tiles += game_map.get_unsafe_moves(ship.position, p_norm)
                            logging.info(
                                "Ship {} submitted {} potential moves".format(ship.id, len(less_dangerous_tiles)))
                            ship_moves[ship.id] = less_dangerous_tiles
                ...
                if SHIP_JOBS.get(ship.id) == "CRASH":
                    ...
                    if distance_to_dock == 1:
                        ...
                        logging.info("Ship {} submitted {} potential moves".format(ship.id, len(unsafe_moves)))
                        if ship_moves.get(ship.id) is None:
                            ship_moves[ship.id] = unsafe_moves
                        else:
                            ship_moves[ship.id] += unsafe_moves
                ...
                if SHIP_JOBS.get(ship.id) == "CLEAN":
                    ...
                    inspired = inspiration_values[ship.position.y][ship.position.x] > 1
                    ...
                    for position in nearby_positions:
                        ...
                        p_norm_inspired = inspiration_values[p_norm.y][p_norm.x] > 1

                        tile_value = game_map[p_norm].halite_amount
                        potential_gain = math.ceil(tile_value * .25) - cost_to_move

                        if p_norm_inspired:
                            potential_gain = math.ceil(tile_value * .75) - cost_to_move
            ...
            while changes_made:
                ...
                for k in ship_moves.keys():
                    ...
                    for move in unsafe_moves:
                        ...
                        # tile is dangerous
                        if ENEMY_RADIUS > 0 and SHIP_JOBS.get(ship.id) not in ["CRASH", "RETURN"]:
                            ...
                            if danger_rating > 0 and danger_rating / 4 > .5 - (ship.halite_amount / constants.MAX_HALITE):
                                continue
            ...
            # spawn ships
            if game.turn_number <= BUILD_UNTIL and halite_pct_remaining > 50 and halite_available >= constants.SHIP_COST and not \
